...
-   IoC и DI - принципе на совмещение которых работет Spring IoC
    ...
    -   DI(Dependency Injection) - инъекция зависимостей. Например, если в конструкторе записать ссылку на другой класс
...
-   [@EnableTransactionManagement - разрешаем транзакции](#spring-data---transactional)
...
-   Как из имен методов получается запрос: [ссылочка на документацию](https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#jpa.query-methods.query-creation)
...
-   в соевременных версиях Spring Boot, не надо, транзакции разрешены по умолчанию
