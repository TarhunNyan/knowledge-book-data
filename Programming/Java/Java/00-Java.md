...
-   Все его поля должны быть Serializable
-   Поля которые сериализовывать не надо - нужно пометить как transient
-   Поля помеченные как static - не сериализуются
-   Чтобы объект можно было сериализовать, он должен - имплементировать интерфейс Serializable
    -   Serializable - просто метка для java
    -   Никаких методов переопределять не надо
...
-   Чтобы объект можно было десериализовать
...
-   Поля помеченные, как transient будут заполнены null
...
## Externalizable - Расширенная сериализация

Расширенная сериализация - расширив объект от интерфейса Externalizable, можно переопределить методы writeExternal и readExternal:
...
writeObject и readObject - есть у Serializable, но это костыль, и в интерфейсе они неописаны а потому не обязательны. Кроме того Serializable работает через великое и могучее reflection api, а потому это медленне чем используя Externalizable
