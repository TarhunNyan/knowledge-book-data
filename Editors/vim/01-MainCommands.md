...
**<find text>** - искомый текст(regexp)

**<replace text>** - текст на который меняем(regex)
...
Обычно у vim'а config лежит в:

```bash
files/home/.vimrc
```

Обычно у neovim'а config лежит в:
...
Использовать файл по текущему пути, как конфиг для vim'а
